...
function CustomerCreate() {
  ...
  const onFinish = async (values: BookingInterface) => {
    ...
    console.log("CheckIn",values.CheckIn);
    console.log("CheckOut",values.CheckOut);
    const bookingData: BookingInterface = {
      CheckIn: values.CheckIn,  // ISO 8601 date string
      CheckOut: values.CheckOut, // ISO 8601 date string
    ...
    if (selectedRoom) {
      const roomUpdate: RoomInterface = {
        ...selectedRoom,
        Status: "Invalid", // Update status to Invalid
      };

      const roomPach = await UpdateRoom(roomUpdate);
      console.log("room update = ", roomPach);
    } else {
